...
# 添加ts2vec目录到Python路径，这样可以正确导入models模块
...
def main():
    ...
    if args.loader == 'forecast_csv':
        ...
        # 只传递数据集名称，不传递完整路径
        dataset_name = 'electricity'  # 假设我们只评估electricity数据集
        data, train_slice, valid_slice, test_slice, scaler, pred_lens, n_covariate_cols = load_forecast_csv(dataset_name)
    ...
    # 导入eval_protocols
    from tasks.forecasting import eval_protocols
    ...
    for pred_len in pred_lens:
        ...
        # 每次完成一个预测长度就保存一次结果，避免被终止时丢失所有数据
        eval_res = {
            'ours': ours_result,
        ...
        print(f"已保存 {len(ours_result)} 个预测长度的评估结果到: {eval_res_path}")
